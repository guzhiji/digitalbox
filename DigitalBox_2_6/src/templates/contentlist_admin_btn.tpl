...
function addContent(){
	window.location="admin_content.php?module=content&class={$classid}&function=edit";
}
function editContent(){
	var cid=getSelectedValue(document.admin_content);
	if(cid>0){
		window.location.href="admin_content.php?module=content&class={$classid}&function=edit&id="+cid;
	}else{
		window.alert("您未选择对象！");
	}
}
function moveContent(){
	...
	if (getSelectedValue(f)>0){
		f.method="post";
		f.action="admin_content.php?module=content&class={$classid}&type={$type_en}&function=beginmove";
		f.submit();
...
function recycleContent(){
	...
	if (getSelectedValue(f)>0){
		if (window.confirm("您真的要回收此{$type_cn}吗？")){
			f.method="post";
			f.action="admin_content.php?module=content&class={$classid}&type={$type_en}&function=recycle";
			f.submit();
		}
	}else window.alert("您未选择对象！");
}
function goBack(){
	window.location="admin_content.php?module=content&channel={$channelid}";
}
//]]>
</script>
