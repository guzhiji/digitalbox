...
/*
  ...
  ------------------------------------------------------------------
 */
require_once("modules/view/TitleList.class.php");
require_once("modules/view/ImageList.class.php");
require_once("modules/data/sql_content.module.php");

class ContentList {

    private $_titleonly, $_extra, $_mode, $_type, $_parentid, $_parentname, $_connid;
    private $_maxlen, $_maxtitles, $_maximages;
    private $_imagelist, $_titlelist;

    function __construct($connid, $checkboxname="") {
        $this->_connid = $connid;
        $this->_mode = "";
        $this->_type = 0;
        $this->_maxlen = 0;
    ...
    public function GetHTML($order, $formore=0, $pagename=NULL) {
        //a, b, current_page, total_page, total_rec, rs, html, mcn
        $html = "";
        $pb = NULL;
        ...
        if ($this->_type == 2 && !$this->_titleonly) {
            ...
            if ($formore == 2) {
                $rs = db_query($this->_connid, $sql->GetCountQuery());
                if ($rs) {
                    $list = db_result($rs);
                    if (isset($list[0])) {
            ...
            $rs = db_query($this->_connid, $sql->GetSelect() . " LIMIT " . (($current_page - 1) * $page_size) . ",$page_size");
            if ($rs) {
                $list = db_result($rs);
                if (count($list) > 0) {
                    //generate list
                    ...
                    switch ($formore) {
                        case 1://show button [more]
                            ...
                            $html = TransformTpl("contentlist_morebtn", array(
                                "HTML" => $html,
                                "TipText" => ($this->_mode == "channel") ? ChannelTip($this->_parentname, "图片") : ClassTip($this->_parentname, "图片", $this->_parentname),
                                "Page" => $pagename,
                                "MoreBtn" => GetResPath("button/more.gif", "images", GetSettingValue("style_id"))
                                    ), __CLASS__);
