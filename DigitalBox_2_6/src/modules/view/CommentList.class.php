...
class CommentList extends ListView {
    ...
    public function Bind($connid, $key="", $page_size=0) {
        ...
        //count
        if ($this->formore == 2) {
            $rs = db_query($connid, "SELECT COUNT(*) FROM guest_info{$where}");
            if ($rs) {
                $list = db_result($rs);
                if (isset($list[0])) {
        ...
        //fetch data
        $rs = db_query($connid, $sql . " LIMIT " . ($page_size * ($page_number - 1)) . ",$page_size");
        if ($rs) {
            $list = db_result($rs);
            if (count($list) > 0) {
