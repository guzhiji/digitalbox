...
abstract class Content_Admin{
	...
	var $_connid,$_passport,$task,$error;
	...
	protected function _check_parentclass($parentfieldname,$type=NULL){
		...
		if($parent_class!=""){
			if($type==NULL){
				$rs=db_query($this->_connid,"select id,class_name from class_info where id=%d",array($parent_class));
			}else{
				$rs=db_query($this->_connid,"select class_info.class_name from class_info,channel_info where channel_info.id=class_info.parent_channel and channel_info.channel_type=%d and class_info.id=%d",array($type,$parent_class));
			}
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
	...
	protected function _check_content($idfieldname,$contenttype,&$contentid,&$contentname){
		...
		if($id!=""){
			$rs=db_query($this->_connid,"select * from {$contenttype}_info where id=%d",array($id));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
					$this->class_id = $list[0]["parent_class"];
	...
	protected function _delete($contenttype,$id){
		$e=FALSE;
		if($this->task=="delete"){
			if(!db_query($this->_connid,"delete from {$contenttype}_info where id=%d",array($id))){
				$e=TRUE;
			}
		}else{
			$e=TRUE;
	...
	protected function _recycle($contenttype,$id){
		...
		if($this->task=="recycle"){
			if(!db_query($this->_connid,"update {$contenttype}_info set parent_class=0 where id=%d",array($id))){
				$e=TRUE;
			}
		}else{
			$e=TRUE;
	...
	protected function _restore($contenttype,$id,$classid){
		...
		if($e){
			...
			$rs=db_query($this->_connid, "SELECT class_info.id,class_info.class_name FROM channel_info,class_info WHERE class_info.parent_channel=channel_info.id AND channel_info.channel_type=%d AND class_info.id=%d",array($type,$classid));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
		...
		if($e){
			...
			$rs=db_query($this->_connid, "SELECT \"true\" FROM {$contenttype}_info WHERE parent_class<1 AND id=%d",array($id));
			if($rs){
				$list=db_result($rs);
				if(!isset($list[0])){
					$e=FALSE;
		...
		if($e){
			//restore
			if(!db_query($this->_connid,"UPDATE {$contenttype}_info SET parent_class=%d WHERE id=%d",array($classid,$id))){
				$e=FALSE;
			}
		}
		if(!$e){
