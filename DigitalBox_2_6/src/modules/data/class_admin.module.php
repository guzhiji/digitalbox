...
class Class_Admin{
	...
	private function _check_add(){
		...
		if($this->channel_id!=""){
			$rs=db_query($this->_connid,"select id,channel_name,channel_type from channel_info where id=%d",array($this->channel_id));
			if($rs){
				$list=db_result($rs);
				if(!isset($list[0])){
					$e=FALSE;
	...
	public function add(){
		$f=FALSE;
		if($this->task == "add"){
			$f=db_query($this->_connid,"insert into class_info (class_name,parent_channel) values (\"%s\",%d)",array($this->class_name,$this->channel_id));
		}
		if(!$f) $this->error.="添加失败;";
		return $f;
	}
	...
	private function _check_save(){
		...
		$class_id = strPost("id");
		if($class_id!=""){
			$rs=db_query($this->_connid,"select
	...
	public function save(){
		$f=FALSE;
		if($this->task == "save"){
			$f=db_query($this->_connid,"update class_info set class_name=\"%s\" where id=%d",array($this->class_name,$this->class_id));
		}
		if(!$f) $this->error.="保存失败;";
		return $f;
	}
	...
	private function _check_delete(){
		...
		if($class_id!=""){
			$rs=db_query($this->_connid,"select channel_info.id as channel_id,channel_name,channel_type,class_name from class_info,channel_info where channel_info.id=class_info.parent_channel and class_info.id=%d",array($class_id));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
					$this->channel_id=$list[0]["channel_id"];
		...
		if($f){
			...
			$rs=db_query($this->_connid, "select id from {$class_type}_info where parent_class=%d",array($class_id));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
					$this->error.="不能删除非空栏目";
	...
	public function delete(){
		$f=FALSE;
		if($this->task == "delete"){
			$f=db_query($this->_connid,"delete from class_info where id=%d",array($this->class_id));
		}
		if(!$f) $this->error.="删除失败;";
		return $f;
	}
}
?>