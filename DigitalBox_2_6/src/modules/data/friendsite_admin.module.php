...
class FriendSite_Admin{
	...
	public function add(){
		if($this->task=="add"){
			return db_query($this->_connid,"insert into friendsite_info (site_name,site_add,site_logo,site_text) values (\"%s\",\"%s\",\"%s\",\"%s\")",array($this->site_name,$this->site_add,$this->site_logo,$this->site_text));
		}
		return FALSE;
	}
	...
	private function _check_delete(){
		...
		$site_id = strPost("id");
		if($site_id!=""){
			$rs=db_query($this->_connid,"select site_name from friendsite_info where id=%d",array($site_id));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
	...
	public function delete(){
		if($this->task=="delete"){
			return db_query($this->_connid,"delete from friendsite_info where id=%d",array($this->site_id));
		}
		return FALSE;
	}
	...
	private function _check_save(){
		...
		$site_id = strPost("id");
		if($site_id!=""){
			$rs=db_query($this->_connid,"select id from friendsite_info where id=%d",array($site_id));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
					$this->site_id=intval($site_id);
	...
	public function save(){
		if($this->task=="save"){
			return db_query($this->_connid,"update friendsite_info set site_name=\"%s\",site_add=\"%s\",site_logo=\"%s\",site_text=\"%s\" where id=%d",array($this->site_name,$this->site_add,$this->site_logo,$this->site_text,$this->site_id));
		}
		return FALSE;
	}
}
