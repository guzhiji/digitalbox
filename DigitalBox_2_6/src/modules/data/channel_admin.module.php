...
class Channel_Admin{
	...
	private function _check_fields(){
		...
		if($this->channel_type > 4 || $this->channel_type < 0){
			...
			if($this->channel_add == ""){
				...
				if(strlen($this->channel_add) > 255) $this->error .= "频道地址不可超过255字符;";
			}
		}
	...
	public function add(){
		...
		if($this->task == "add"){
			if($this->channel_add != ""){
				...
				$f=db_query($this->_connid,$sql, array($this->channel_name,$this->channel_type,$this->channel_add));
			}
		}
	...
	private function _check_save(){
		...
		if($channel_id!=""){
			$rs=db_query($this->_connid,"select channel_type from channel_info where id=%d",array($channel_id));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
	...
	public function save(){
		...
		if($this->task == "save"){
			if($this->channel_type == 0){
				...
				$f=db_query($this->_connid, $sql,array($this->channel_name,$this->channel_add,$this->channel_id));
			}else{
				$sql = "UPDATE channel_info SET channel_name=\"%s\" WHERE id=%d";
				$f=db_query($this->_connid, $sql,array($this->channel_name,$this->channel_id));
			}
	...
	private function _check_delete(){
		...
		if($channel_id!=""){
			$rs=db_query($this->_connid,"select id,channel_name,channel_type from channel_info where id=%d",array($channel_id));
			if($rs){
				$list=db_result($rs);
				if(isset($list[0])){
					$this->channel_id=intval($channel_id);
		...
		$rs=db_query($this->_connid, "select id from class_info where parent_channel=%d",array($channel_id));
		if($rs){
			$list=db_result($rs);
			if(isset($list[0])){
				$this->error.="不能删除非空频道;";
	...
	public function delete(){
		$f=FALSE;
		if($this->task == "delete"){
			$f=db_query($this->_connid,"DELETE FROM channel_info WHERE id=%d",array($this->channel_id));
		}
