...
class User_Admin{
	...
	public function changePWD(){
		if($this->task=="changepwd"){
			return db_query($this->_connid,"update admin_info set admin_PWD=\"%s\" where admin_UID=\"%s\"",array($this->new_pwd,strSession("Admin_UID")));
		}
		return FALSE;
	}
	...
	private function _check_add(){
		...
		if($this->_passport->doubleCheck(TRUE)){
			...
			if($this->error==""){
				$rs=db_query($this->_connid,"select admin_UID from admin_info where admin_UID=\"%s\"",array($admin_uid));
				if($rs){
					$list=db_result($rs);
					if(isset($list[0])){
						$this->error .= "此用户名已经存在;";
	...
	public function add(){
		if($this->task=="add"){
			return db_query($this->_connid,"insert into admin_info (admin_UID,admin_PWD) values (\"%s\",\"%s\")",array($this->admin_uid,$this->admin_pwd));
		}
		return FALSE;
	}
	...
	public function delete(){
		if($this->task=="delete"){
			return db_query($this->_connid,"delete from admin_info where admin_UID=\"%s\"",array($this->admin_uid));
		}
		return FALSE;
	}
	...
	public static function PWDCheck($password,$repeat,$username=""){
		...
		if($repeat != $password) $error .= "密码确认与新密码不一样;";

		if($username!=""){
			if($username == $password) $error .= "安全起见，用户名与密码不应相同;";
		}
