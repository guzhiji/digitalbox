<form name="edit_media" method="post"
    ...
    <div style="text-align: left; padding-top: 5px;">媒体名称：<input
            class="textinput5" type="text" name="media_name"
            value="{$Content_Name}" /><input type="hidden" name="id"
            value="{$Content_ID}" /></div>
    <div style="text-align: left; padding-top: 5px;">媒体地址：<input
            class="textinput4" type="text" name="media_add" value="{$Content_Addr}" />
        <input type="button" value="文件" class="button1"
               onclick="window.open('admin_editor_uploadlist.php','选择文件','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=500')" />
