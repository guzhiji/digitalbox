...
require("modules/boxfactories/ArticleBoxFactory.class.php");
...
if (!$rs) {
    ...
    if (isset($list[0])) {
        ...
        global $_articleText;
        $_articleAuthor = $item["article_author"];
        $_articleText = $item["article_text"];

        //add 1 to visitorcount
        db_query("UPDATE article_info SET visitor_count=visitor_count+1 WHERE id=%d", array($id));
        $_contentVCount++;
    } else {
        $_error = TRUE;
    }
...
if (!$_error) {
    ...
    $portalpage->AddBoxFactory(new ArticleBoxFactory());
} else {
