...
class Software_Admin extends Content_Admin{
	...
	protected function _check_add(){
		...
		if(!$this->_check_fields()) return FALSE;
	...
	public function add(){
		if($this->task=="add"){
			if(!db_query($this->_connid,"insert into software_info (
					software_HTML,
					visitor_count,
					parent_class,
					software_name,
					...
					software_time,
					software_text
					) values (\"%s\",0,%d,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",%d,\"%s\",%d,\"%s\",\"%s\")",
			array(
			$this->software_HTML,
			$this->class_id,
			$this->software_name,
	...
	public function save(){
		if($this->task=="save"){
			if(!db_query($this->_connid,"update software_info set
					software_HTML=\"%s\",
					software_name=\"%s\",
					software_add=\"%s\",
					software_type=\"%s\",
