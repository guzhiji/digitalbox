...
function validateChannel($id, $type) {
    ...
    $r = new PHPCacheReader(GetCachePath(), 'navigation');
...
function updateNavigation() {
    ...
    try {
        ...
        $ce = new PHPCacheEditor(GetCachePath(TRUE), 'navigation');

        $rs = db_query("select * from channel_info");
        if ($rs) {
            $chlist = db_result($rs);
            foreach ($chlist as $ch)
                $ce->SetValue('channel_'.$ch['id'], $ch, 0, TRUE);
            db_free($rs);
        }
        ...
        if ($rs) {
            ...
            foreach ($clslist as $cls)
                $ce->SetValue('class_'.$cls['id'], $cls, 0, TRUE);
            db_free($rs);
        }
