...
class FileListReader extends DataList {
    ...
    public function LoadList() {
        ...
        if ($this->_extName != "")
            $sql->AddEqual("filExtName", $this->_extName, IBC1_DATATYPE_PURETEXT, IBC1_LOGICAL_AND);
        if ($this->_name != "") {
            if ($this->_nameExact)
                $sql->AddEqual("filName", $this->_name, IBC1_DATATYPE_PURETEXT, IBC1_LOGICAL_AND);
            else
                $sql->AddLike("filName", $this->_name, IBC1_DATATYPE_PURETEXT, IBC1_LOGICAL_AND);
        }
        if ($this->_type != "")
            $sql->AddEqual("filType", $this->_type, IBC1_DATATYPE_PURETEXT, IBC1_LOGICAL_AND);
        if ($this->_uid != "")
            $sql->AddEqual("filUID", $this->_uid, IBC1_DATATYPE_PURETEXT, IBC1_LOGICAL_AND);
        if ($this->_dir > 0) {
