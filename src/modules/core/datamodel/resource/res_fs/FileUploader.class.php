...
class FileUploader extends DataModel {
    ...
    public function SaveFiles() {
        ...
        foreach ($_FILES as $NewFile) {
            ...
            $sql->AddValue("filName", $fNameArray[0], IBC1_DATATYPE_PURETEXT);
            $sql->AddValue("filType", $NewFile["type"], IBC1_DATATYPE_PURETEXT);
            $sql->AddValue("filSize", $NewFile["size"], IBC1_DATATYPE_INTEGER);
            $sql->AddValue("filExtName", $fNameArray[1], IBC1_DATATYPE_PURETEXT);
            $sql->AddValue("filTime", "CURRENT_TIMESTAMP()", IBC1_DATATYPE_EXPRESSION);
            $sql->AddValue("filUID", $this->uid, IBC1_DATATYPE_PURETEXT);
    ...
    private function GetFreeDir($path) {
        ...
        $sql->AddEqual("filUID", $this->uid, IBC1_DATATYPE_PURETEXT);
        $sql->Execute();
        $m = $sql->Fetch(1)->m;
        $sql->CloseSTMT();
        $sql = $conn->CreateSelectSTMT("ibc1_res" . $this->GetServiceName() . "_file");
