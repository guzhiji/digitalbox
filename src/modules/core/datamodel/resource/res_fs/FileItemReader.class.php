...
class FileItemReader extends DataItem {
	...
	public function OpenService(DBConnProvider $Conns, $ServiceName) {
		...
		$c = $this->GetDBConn();
		$sql = $c->CreateSelectSTMT("ibc1_datamodel_resource");
		$sql->AddField("*");
	...
	public function Open($id, $uid="") {
		...
		if ($uid != "")
		$sql->AddEqual("filUID", $uid, IBC1_DATATYPE_PURETEXT, IBC1_LOGICAL_AND);
		$sql->Execute();
		$r = $sql->Fetch(1);
		$sql->CloseSTMT();
		if ($r) {
			$this->id = $r->filID;
			$this->SetValue("filName", $r->filName, IBC1_DATATYPE_PURETEXT);
			$this->SetValue("filType", $r->filType, IBC1_DATATYPE_PURETEXT);
			$this->SetValue("filExtName", $r->filExtName, IBC1_DATATYPE_PURETEXT);
			$this->SetValue("filTime", $r->filTime, IBC1_DATATYPE_PURETEXT);
			$this->SetValue("filUID", $r->filUID, IBC1_DATATYPE_PURETEXT);
