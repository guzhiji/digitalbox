...
class FileItemEditor extends DataItem {
    ...
    /*
      ...
      {
      $this->SetValue("filExtName",$extname,IBC1_DATATYPE_PURETEXT);
      }
      public function SetTime($time)
      {
      $this->SetValue("filTime",$time,IBC1_DATATYPE_PURETEXT);
      }
      public function SetUser($uid)
      {
      $this->SetValue("filUID",$uid,IBC1_DATATYPE_PURETEXT);
      }
    ...
    public function Delete() {
        ...
        $sql = $conn->CreateSelectSTMT("ibc1_datamodel_resource");
        $sql->AddField("Root");
        $sql->AddEqual("ServiceName", $this->GetServiceName(), IBC1_DATATYPE_PURETEXT);
        $sql->Execute();
        $r = $sql->Fetch(1);
        $sql->CloseSTMT();
        if (!$r) {
