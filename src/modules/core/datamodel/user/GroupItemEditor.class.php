...
LoadIBC1Class('DataItem', 'datamodel');
...
class GroupItemEditor extends DataItem {
    ...
    /**
     * prepares to create a new user group
     * 
    ...
    /**
     * prepares to modify an existing user group
     * 
    ...
    public function SetOwner($uid) {
        ...
        if ($uid == '') {
            ...
            if ($this->IsNew) {
                ...
                $sql = $conn->CreateSelectSTMT($this->GetDataTableName('groupuser'));
                $sql->AddField('gpuUID');
    ...
    public function Save() {
        ...
        if ($this->IsNew) {//create a new user group
            ...
            //insert
            $conn = $this->GetDBConn();
            $sql = $conn->CreateInsertSTMT($this->GetDataTableName('group'));
            ...
            //save owner
            if ($this->GetValue('grpOwner') != NULL) {
                $sql = $conn->CreateInsertSTMT($this->GetDataTableName('groupuser'));
            ...
            //update
            $conn = $this->GetDBConn();
            $sql = $conn->CreateUpdateSTMT($this->GetDataTableName('group'));
            while (list($key, $item) = $this->GetEach()) {
                $sql->AddValue($key, $item[0], $item[1]);
            }
    ...
    public function AddUser($uid) {
        ...
        //TODO add foreign references for UID, GID
    ...
    public function RemoveUser($uid) {
        ...
        $sql = $conn->CreateDeleteSTMT($this->GetDataTableName('groupuser'));
        $sql->AddEqual('gpuUID', $uid, IBC1_DATATYPE_PURETEXT);
        $sql->AddEqual('gpuGID', $this->ID, IBC1_DATATYPE_INTEGER, IBC1_LOGICAL_AND);
        $sql->Execute();
        $sql->CloseSTMT();
