...
class DataService {
    ...
    function __construct($ServiceName, $ServiceType) {
        ...
        $dataservices = &$GLOBALS['IBC1_DATASERVICES'];
        if (
                !isset($dataservices) ||
                !isset($dataservices[$ServiceName]) ||
                !isset($dataservices[$ServiceName]['type']) ||
                $dataservices[$ServiceName]['type'] != $ServiceType
        )
            throw new Exception("cannot find the data service $ServiceName of $ServiceType model", 1);

        if (isset($dataservices[$ServiceName]['extra']))
            $this->extra_args = $dataservices[$ServiceName]['extra'];
